...
package cloud::flexibleengine::dds::mode::diskio;
...
my %metrics_mapping = (
    'mongo037_read_throughput' => {
        'output' => 'Disk Read Throughput',
        'label' => 'disk-read',
        'nlabel' => 'dds.disk.read.bytes.sec',
    },
    'mongo038_write_throughput' => {
        'output' => 'Disk Write Throughput',
        'label' => 'disk-write',
        'nlabel' => 'dds.disk.write.bytes.sec',
    }
);
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'metrics', type => 3, cb_prefix_output => 'prefix_metric_output', cb_long_output => 'long_output',
          message_multiple => 'All DiskIO metrics are ok', indent_long_output => '    ',
            group => [
    ...
    foreach my $metric (keys %metrics_mapping) {
        my $entry = {
            ...
            set => {
                ...
                output_template => $metrics_mapping{$metric}->{output} . ': %.2fB/s',
                perfdatas => [
                    { value => $metric , unit => 'B/s', min => 0, label_extra_instance => 1, instance_use => 'display'}
                ],
            }
        };
...
Check DDS instances Network metrics.

Example: 
perl centreon_plugins.pl --plugin=cloud::flexibleengine::dds::plugin  --mode=network --region='eu-west-0'
...
Valid input is network-out, network-in
