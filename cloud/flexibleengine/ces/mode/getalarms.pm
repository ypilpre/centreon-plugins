...
package cloud::flexibleengine::ces::mode::getalarms;
...
sub custom_state_output {
    my ($self, %options) = @_;
    my $msg = sprintf(
        'alarm [name: %s] [state: %s] [metric: %s]  %s',
        $self->{result_values}->{alarm_name},
        $self->{result_values}->{state_value}, $self->{result_values}->{metric_name}, 
         centreon::plugins::misc::change_seconds(value => $self->{result_values}->{last_update})
    );
    return $msg;
}
...
sub custom_state_calc {
    ...
    $self->{result_values}->{state_value} = $options{new_datas}->{$self->{instance} . '_alarmstate'};
...
sub set_counters {
    ...
    $self->{maps_counters}->{alarm} = [
        { label => 'state', threshold => 0, set => {
                key_values => [ { name => 'alarmname' }, { name => 'alarmstate' }, { name => 'metricname' }, { name => 'lastupdate' } ],
...
sub new {
    ...
    $options{options}->add_options(arguments => {
        ...
        'warning-state:s'    => { name => 'warning_state', default => '%{state_value} =~ /insufficient_data/i' },
        'critical-state:s'   => { name => 'critical_state', default => '%{state_value} =~ /alarm/i' },
        'memory'              => { name => 'memory' },
    });
...
sub manage_selection {
    ...
    my $alarm_results = $options{custom}->api_list_ces_alarms();

    my $last_time;
    if (defined($self->{option_results}->{memory})) {
        $self->{statefile_cache}->read(statefile => 'cache_ces_' . $self->{mode} . '_' . $self->{option_results}->{region});
        $last_time = $self->{statefile_cache}->get(name => 'last_time');
    }

    my ($i, $current_time) = (1, time());
    foreach my $alarm (@{$alarm_results->{metric_alarms}}) {        
        my $create_time = $alarm->{update_time}/1000;
        if (!defined($create_time)) {
            $self->{output}->output_add(
                severity => 'UNKNOWN',
                short_msg => "Can't Parse date '" . $alarm->{update_time}/1000 . "'"
            );
            next;
        }
        ...
        if (defined($self->{option_results}->{filter_alarm_name}) && $self->{option_results}->{filter_alarm_name} ne '' &&
            $alarm->{alarm_name} !~ /$self->{option_results}->{filter_alarm_name}/) {
            $self->{output}->output_add(long_msg => "skipping '" . $alarm->{alarm_name} . "': no matching filter.", debug => 1);
            next;
        }
        ...
        $self->{alarms}->{global}->{alarm}->{$i} = { 
           ...
           alarmstate => $alarm->{alarm_state},
            metricname =>  $alarm->{metric}->{metric_name},
...
Set warning threshold for status (Default: '%{state_value} =~ /alarm-5min-windows/i')
