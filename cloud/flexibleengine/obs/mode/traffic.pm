...
package cloud::flexibleengine::obs::mode::traffic;
...
my %metrics_mapping = (
    'download_bytes' => {
        'output' => 'Bytes Download',
        'label' => 'bytes-download',
        'nlabel' => 'obs.download.bytes',
        'unit' => 'B'
    },
    'upload_bytes' => {
        'output' => 'Bytes Upload',
        'label' => 'bytes-upload',
        'nlabel' => 'obs.upload.bytes',
        'unit' => 'B'
    },
...
Check OBS Buckets Traffic metrics.

Example: 
perl centreon_plugins.pl --plugin=cloud::flexibleengine::obs::plugin  --mode=traffic --region='eu-west-0'
 --bucket-name='my-bucket' --filter-metric='Download' --statistic='average'
--critical-download-bytes='10' --verbose
...
Filter metrics (Can be: 'download_bytes', 'upload_bytes').
(Can be a regexp).

=item B<--warning-*> B<--critical-*>

Thresholds warning (Can be 'bytes-uploaded, 'bytes-downloaded').
