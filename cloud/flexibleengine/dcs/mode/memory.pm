...
package cloud::flexibleengine::dcs::mode::memory;
...
my %metrics_mapping = (
    ...
    'memory_usage' => {
        ...
        'output' => 'Memory Usage',
        'label' => 'memory-usage',
        'nlabel' => 'dcs.memory.utilization.percentage',
        'unit' => '%'
    },
);
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'metrics', type => 3, cb_prefix_output => 'prefix_metric_output', cb_long_output => 'long_output',
          message_multiple => 'All Memory metrics are ok', indent_long_output => '    ',
            group => [
    ...
    foreach my $metric (keys %metrics_mapping) {
        my $entry = {
            ...
            set => {
                ...
                perfdatas => [
                    { value => $metric , unit => $metrics_mapping{$metric}->{unit}, min => 0, label_extra_instance => 1, instance_use => 'display'}
                ],
            }
        };
...
sub check_options {
    ...
    if (lc $self->{option_results}->{engine} eq 'redis'){
        $self->{dimension_name} = 'dcs_instance_id';
    }

    if (lc $self->{option_results}->{engine} eq 'memcached'){
        $self->{dimension_name} = 'dcs_memcached_instance_id';
    }
...
Check DCS instances Memory metrics.

Example: 
perl centreon_plugins.pl --plugin=cloud::flexibleengine::dcs::plugin  --mode=memory --region='eu-west-0'
--engine='redis' --instance_id='30191908-bf15-44e8-bf5c-ffa121bbd4ea' --filter=average
--critical-memory-utilization=80 --verbose
...
Set the instance id (redis or memcached) (Required).

=item B<--warning-*> B<--critical-*>

Thresholds warning & critical.
Valid input is memory-utilization
