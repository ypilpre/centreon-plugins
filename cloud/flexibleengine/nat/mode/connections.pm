...
package cloud::flexibleengine::nat::mode::connections;
...
my %metrics_mapping = (
    'snat_connection' => {
        'output' => 'SNAT connections',
        'label' => 'snat-connections',
        'nlabel' => 'nat.snat.connections',
        'unit' => ''
    }
...
sub long_output {
    ...
    return "Checking NAT '" . $options{instance_value}->{display} . "' ";
}
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'metrics', type => 3, cb_prefix_output => 'prefix_metric_output', cb_long_output => 'long_output',
          message_multiple => 'All NAT metrics are ok', indent_long_output => '    ',
            group => [
...
sub manage_selection {
    ...
    foreach my $instance (@{$self->{ces_instance}}) {
        $metric_results{$instance} = $options{custom}->api_cloudeyes_get_metric(
            namespace => 'SYS.NAT',
            dimensions => [ { name => 'nat_gateway_id', value => $instance } ],
