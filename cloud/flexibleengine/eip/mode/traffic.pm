...
package cloud::flexibleengine::eip::mode::traffic;
...
my %metrics_mapping = (
    'downstream_bandwidth' => {
        ...
        'nlabel' => 'eip.bandwidth.in.bitpersecond',
        'unit' => 'b/s'
    },
    'upstream_bandwidth' => {
        'output' => 'Upstream Bandwidth',
        'label' => 'network-inband-out-bytes-rate',
        'nlabel' => 'eip.bandwidth.out.bitpersecond',
        'unit' => 'b/s',
    }
...
sub prefix_metric_output {
    ...
    return "eip '" . $options{instance_value}->{display} . "' ";
}
...
sub long_output {
    ...
    return "Checking EIP '" . $options{instance_value}->{display} . "' ";
}
...
sub manage_selection {
    ...
    foreach my $instance (@{$self->{ces_instance}}) {
        $metric_results{$instance} = $options{custom}->api_cloudeyes_get_metric(
            namespace => 'SYS.VPC',
            dimensions => [ { name => 'publicip_id', value => $instance } ],
...
Check eip instances network metrics.

Example: 
perl centreon_plugins.pl --plugin=cloud::flexibleengine::eip::plugin  --mode=traffic --region='eu-west-0'
